...
fronting = 1-norm.cdf(z, loc=0, scale=3) # fronting is constant, cannot be conformal.
first_lay_e0 = norm.cdf(z, loc=0, scale=3) * (1-norm.cdf(z, loc=20, scale=2))
second_lay_e0 = norm.cdf(z, loc=0, scale=3) * norm.cdf(z, loc=20, scale=2) * (1-norm.cdf(z, loc=20+30, scale=4))
third_lay_e0 = norm.cdf(z, loc=0, scale=3) * norm.cdf(z, loc=20, scale=2) * (norm.cdf(z, loc=20+30, scale=4)) * (1-(norm.cdf(z, loc=20+30+15, scale=1.5)))
backing_e0 = 1 - np.sum((fronting, first_lay_e0, second_lay_e0, third_lay_e0), axis=0)

# conformal examples
# conformal = (0, 1, 0, 0)
# conformal interfaces are = interface before but translated in z.
f0_st1 = norm.cdf(z, loc=0+20, scale=3) # name means interface 0 (f0), (s)hifted by (t)hickness of layer (1)
# volume fraction of layer with conformal interface is (1-conformal interface) - everything before this layer
first_lay_e1 = (1-f0_st1) - fronting
# vf of layers after conformal interface is as we calc above but now the conformal interface is where we calc from.
second_lay_e1 = f0_st1 * (1-norm.cdf(z, loc=20+30, scale=4))
third_lay_e1 = f0_st1 * norm.cdf(z, loc=20+30, scale=4) * (1-(norm.cdf(z, loc=20+30+15, scale=1.5)))
backing_e1 = 1 - np.sum((fronting, first_lay_e1, second_lay_e1, third_lay_e1), axis=0)
...
first_lay_e2 = norm.cdf(z, loc=0, scale=3) * (1-norm.cdf(z, loc=20, scale=2))
# second_lay's interface is conformal to the combination of all interfaces before it, 
# so translate the first and second layer interfaces by the thickness of layer 2.
...
third_lay_e2 = (f0_st2 * f1_st2) * (1-(norm.cdf(z, loc=20+30+15, scale=1.5)))
backing_e2 = 1 - np.sum((fronting_e2, first_lay_e2, second_lay_e2, third_lay_e2), axis=0)
...
first_lay_e3 = norm.cdf(z, loc=0, scale=3) * (1-norm.cdf(z, loc=20, scale=2))
second_lay_e3 = norm.cdf(z, loc=0, scale=3) * norm.cdf(z, loc=20, scale=2) * (1-norm.cdf(z, loc=20+30, scale=4))
# second_lay's interface is conformal to the combination of all interfaces before it, 
# so translate the first and second layer interfaces by the thickness of layer 2.
f0_st3 = norm.cdf(z, loc=0+15, scale=3)
f1_st3 = norm.cdf(z, loc=0+20+15, scale=2)
f2_st3 = norm.cdf(z, loc=0+20+30+15, scale=4)
third_lay_e3 = (1 - (f0_st3 * f1_st3 * f2_st3)) - (fronting_e3 + first_lay_e3 + second_lay_e3)
backing_e3 = 1 - np.sum((fronting_e3, first_lay_e3, second_lay_e3, third_lay_e3), axis=0)
...
backing_e4 = 1 - np.sum((fronting_e4, first_lay_e4, second_lay_e4, third_lay_e4), axis=0)

# conformal = (0, 0, 1, 1)
end = 82 # 5 + (0 + 20 + 30 + 15) + 4 * 3 = 82
z = np.linspace(start, end, int((end - start) / 0.5) + 1)
fronting_e5 = 1-norm.cdf(z, loc=0, scale=3)
first_lay_e5 = norm.cdf(z, loc=0, scale=3) * (1-norm.cdf(z, loc=20, scale=2))
...
third_lay_e5 = (1-(f0_st2t3*f1_st2t3)) - (fronting_e5 + first_lay_e5 + second_lay_e5)
backing_e5 = 1 - np.sum((fronting_e5, first_lay_e5, second_lay_e5, third_lay_e5), axis=0)
...
backing_e6 = 1 - np.sum((fronting_e6, first_lay_e6, second_lay_e6, third_lay_e6), axis=0)
...
standard_examples_conformal = [
    ...
    (
        (3, 'conformal', 4, 1.5),
        np.vstack((fronting, first_lay_e1, second_lay_e1, third_lay_e1, backing_e1))  
    ),
    (
        (3, 2, 'conformal', 1.5),
        np.vstack((fronting_e2, first_lay_e2, second_lay_e2, third_lay_e2, backing_e2))    
    ),
    (
        (3, 2, 4, 'conformal'),
        np.vstack((fronting_e3, first_lay_e3, second_lay_e3, third_lay_e3, backing_e3))   
    ),
    (
        (3, 'conformal', 'conformal', 1.5),
        np.vstack((fronting_e4, first_lay_e4, second_lay_e4, third_lay_e4, backing_e4))   
    ),
    (
        (3, 2, 'conformal', 'conformal'),
        np.vstack((fronting_e5, first_lay_e5, second_lay_e5, third_lay_e5, backing_e5))   
    ),
    (
        (3, 'conformal', 4, 'conformal'),
        np.vstack((fronting_e6, first_lay_e6, second_lay_e6, third_lay_e6, backing_e6))   
    ),
    (
        (3, 'conformal', 'conformal', 'conformal'),
        np.vstack((fronting_e7, first_lay_e7, second_lay_e7, third_lay_e7, backing_e7))   
    ),
]
