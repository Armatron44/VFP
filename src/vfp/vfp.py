...
class refnxVFP(Component, BaseVFP):
    ...
    SLD profiles are calculated via generating volume fraction profiles, 
    which avoid negative volume fractions while keeping the total volume fraction = 1. 
    This object should be passed to a refnx.reflect.Structure, 
    with the fronting and backing refnx.reflect.SLD surrounding the refnxVFP.
    
    Parameters
    ----------
    nucSLDs : array / tuple / list of floats or Parameters    
        Nuclear scattering length densities of each layer within the volume fraction profile.
    thicknesses : array / tuple / list of floats or Parameters    
        Thicknesses of layers - These control the midpoint-to-midpoint width of a layer's volume fraction
        profile.
    roughnesses : array / tuple / list of floats or Parameters    
        Roughnesses of layers - These control the width of interfaces between two adjacent layers in the
        volume fraction profile.
    nuciSLDs : None or an array / tuple / list of floats or Parameters - optional
        Imaginary scattering length densities of each layer within the volume fraction profile.
    magSLDs : None or an array / tuple / list of floats or Parameters - optional  
    ...
    demaglocs : None or an array / tuple / list of floats or Parameters - optional 
        ...
        + parameter 2 value.
    demagwidths : None or an array / tuple / list of floats or Parameters - optional
        If supplied must either be a list of 2 or 4 parameters.
    ...
    def __init__(
        ...
        thicknesses: tuple[float, Parameter] | list[float, Parameter] | np.ndarray,
        roughnesses: tuple[float, Parameter] | list[float, Parameter] | np.ndarray,
        nuciSLDs: None | tuple[float, Parameter] | list[float, Parameter] | np.ndarray = None,
        magSLDs: None | tuple[float, Parameter] | list[float, Parameter] | np.ndarray = None,
        spin_state: str = 'none',
        orientation: str = 'front',
        demaglocs: None | tuple[float, Parameter] | list[float, Parameter] | np.ndarray = None,
        demagwidths: None | tuple[float, Parameter] | list[float, Parameter] | np.ndarray = None,
        ...
        self.thicknesses = [possibly_create_parameter(j, 
                                                      name=f"{self.name} - Layer{i} - thick"
                                                      ) for i, j in enumerate(thicknesses)]
        
        self.demaglocs = [possibly_create_parameter(j, 
                                                    name=f"{self.name} - demaglocs{i}"
                                                    ) for i, j in enumerate(demaglocs)]
        
        self.demagwidths = [possibly_create_parameter(j, 
                                                      name=f"{self.name} - demagwidths{i}"
                                                      ) for i, j in enumerate(demagwidths)]
        
        # create parameters for any roughness values that aren't parameters
        self.roughnesses_p = [possibly_create_parameter(j, 
                                                        name=f"{self.name} - Layer{i}/Layer{i+1} - rough"
    ...
    def slabs(self, 
              structure: Structure | None = None) -> np.ndarray:
        """
...
if REFL1D_IMPORTED:
    class refl1dVFP(Layer, BaseVFP):
        ...
        SLD profiles are calculated via generating volume fraction profiles, 
        which avoid negative volume fractions while keeping the total volume fraction = 1. 
        This object should be passed to a refl1d.model.Stack, with the fronting and backing refl1d.material.SLD
        ...
        """
        def __init__(self, 
                    nucSLDs: tuple[float, bumpsParameter] | list[float, bumpsParameter] | np.ndarray, 
                    thicknesses: tuple[float, bumpsParameter] | list[float, bumpsParameter] | np.ndarray,
                    roughnesses: tuple[float, bumpsParameter] | list[float, bumpsParameter] | np.ndarray, 
                    nuciSLDs: None | tuple[float, bumpsParameter] | list[float, bumpsParameter] | np.ndarray = None,
                    magSLDs: None | tuple[float, bumpsParameter] | list[float, bumpsParameter] | np.ndarray = None,
                    spin_state: str = 'none',
                    orientation: str = 'front',
                    demaglocs: None | tuple[float, bumpsParameter] | list[float, bumpsParameter] | np.ndarray = None,
                    demagwidths: None | tuple[float, bumpsParameter] | list[float, bumpsParameter] | np.ndarray = None,
                    SLD_constraint: None | Callable = None, 
                    max_delta_z: float = 0.5
        ) -> None:
        ...
        def layer_parameters(self) -> dict:
            ...
            keys_idx = [i for i, j in enumerate([self.thicknesses, 
                                                self.roughnesses_p, 
                                                self.demaglocs, 
                                                self.demagwidths, 
                                                self.nucSLDs_p, 
