...
def surfaces_for_display(VFP: "BaseVFP", points: int = 50) -> np.ndarray:
    """
    Produces a 2D array of random variates, 
    drawn from the distributions which describe each interface.
    ...
    if VFP.orientation in ("front"):
        ...
        if num_conform != 0:
            for i in idx_where_conformal:
                interf_arr[i] = (
                    np.min(interf_arr[i + 1 :].T, axis=1) - thicks[::-1][i]
                )
...
def model_plot(
    ...
    if microslice_SLD:
        ...
        if total_SLD:
            if VFP.spin_state in ("none"):
                ...
                ax[0].plot(
                    ...
                    tot_sld,
                    color="k",
                    label=r"$\mathrm{SLD}_{\mathrm{n}} - \mathrm{SLD}_{\mathrm{m}}$",
                )
        ...
        if total_SLD:
            if VFP.spin_state in ("none"):
                ax[0].plot(
                    z, tot_sld, color="k", label=r"$\mathrm{SLD}_{\mathrm{n}}$"
                )
            ...
            ax[0].plot(
                z, SLDn, color="k", label=r"$\mathrm{SLD}_{\mathrm{n}}$"
            )
        ...
        for i, j in enumerate(vfs):
            if i == 0:
                ax[1].plot(
                    ...
                    label="Backing",
                    color=colours[
                        (2 * len(VFP.thicks) - 2 * i) % len(colours)
                    ],
                    zorder=len(VFP.thicks) - i,
                )
    ...
    if VFP.orientation in ("front"):
        ...
        for i, j in enumerate(surfaces):  # do the surfaces
            ax[2].plot(
                ...
                range(0, points),
                marker=".",
                color=colours[(len(VFP.thicks) - 2 * i) % len(colours)],
                zorder=(len(VFP.thicks) + 1 + 2 * i),
            )
        ...
        for i in range(0, len(VFP.thicks) + 1):  # then do the fills
            if i == 0:
                ax[2].fill_betweenx(
                    ...
                    interpolate=True,
                    color=colours[
                        (2 * len(VFP.thicks) + 1 - 2 * i) % len(colours)
                    ],
                    zorder=2 * i,
                )
