...
class VFP(Component):
    ...
    def __init__(
        ...
        if self.contrast == 'D2O_up':
            self.nSLDs = np.array([nSLDs[i] for i in [0, 1, 2, 3, 4]])
            self.niSLDs = np.array([niSLDs[i] for i in [0, 1, 2, 3, 4]])
        elif self.contrast == 'D2O_down':
            self.nSLDs = np.array([nSLDs[i] for i in [0, 1, 2, 3, 4]])
            self.niSLDs = np.array([niSLDs[i] for i in [0, 1, 2, 3, 4]])
        elif self.contrast == 'H2O_up':
            self.nSLDs = np.array([nSLDs[i] for i in [0, 1, 2, 3, 5]])
            self.niSLDs = np.array([niSLDs[i] for i in [0, 1, 2, 3, 5]])
        elif self.contrast == 'H2O_down':
            self.nSLDs = np.array([nSLDs[i] for i in [0, 1, 2, 3, 5]])
            self.niSLDs = np.array([niSLDs[i] for i in [0, 1, 2, 3, 5]])
    ...
    def get_vfs(cls, rough, thick, ex, mxdz):
        ...
        cls.vfs = np.array([cls.get_erf(0, loc, rough, thick), #Si
                            (1-cls.get_erf(0, loc, rough, thick))*cls.get_erf(1, loc, rough, thick), #SiO2
                            (1-cls.get_erf(0, loc, rough, thick))*(1-cls.get_erf(1, loc, rough, thick))*cls.get_erf(2, loc, rough, thick), #Fe
                            (1-cls.get_erf(0, loc, rough, thick))*(1-cls.get_erf(1, loc, rough, thick))*(1-cls.get_erf(2, loc, rough, thick))*cls.get_erf(3, loc, rough, thick), #FeOx
                            (1-cls.get_erf(0, loc, rough, thick))*(1-cls.get_erf(1, loc, rough, thick))*(1-cls.get_erf(2, loc, rough, thick))*(1-cls.get_erf(3, loc, rough, thick)) #solv
                            ])
        return cls.vfs
    ...
    def init_demag(cls, rough, thick, locs, widths):
        ...
        demag_arr = np.vstack((no_demag_factor, no_demag_factor, no_demag_factor, no_demag_factor, no_demag_factor)) #as we have 5 layers, we must supply 5 demag_factors.
    ...
    def get_slds(self):
        ...
        if self.contrast in ('D2O_up', 'H2O_up'):
            tot_sld = sum_sldn_list + sum_sldm_list
        elif self.contrast in ('D2O_down', 'H2O_down'):
            tot_sld = sum_sldn_list - sum_sldm_list
