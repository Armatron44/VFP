...
class VFP(Component):
    ...
    def __init__(
        ...
        if self.contrast == 'D2O':
            self.nSLDs = np.array([nSLDs[i] for i in [0, 1, 2, 3]])
            self.niSLDs = np.array([niSLDs[i] for i in [0, 1, 2, 3]])
    ...
    def get_vfs(cls, rough, thick, ex, mxdz):
        ...
        cls.vfs = np.array([cls.get_erf(0, loc, rough, thick), #Si
                            ...
                            (1-cls.get_erf(0, loc, rough, thick))*(1-cls.get_erf(1, loc, rough, thick))*cls.get_erf(2, loc, rough, thick), #Surf
                            (1-cls.get_erf(0, loc, rough, thick))*(1-cls.get_erf(1, loc, rough, thick))*(1-cls.get_erf(2, loc, rough, thick)) #Solv
                            ])
        return cls.vfs
    ...
    def init_demag(cls, rough, thick, locs, widths):
        ...
        demag_arr = np.vstack((no_demag_factor, no_demag_factor, no_demag_factor, no_demag_factor)) #as we have 4 layers, we must supply 4 demag_factors.
    ...
    def get_slds(self):
        ...
        if self.contrast in ('D2O'):
            tot_sld = sum_sldn_list + sum_sldm_list
        # elif self.contrast in ('dd_down', 'hd_down'):
            # tot_sld = sum_sldn_list - sum_sldm_list
