...
class VFP(Component):
    ...
    def __init__(
        ...
        magSLDs_p = []
        if self.magSLDs.any(): # check if magSLDs doesn't only contain zeros - if it does, we don't need parameters.
            for k, i in enumerate(self.magSLDs):
                if isinstance(i, Parameter):
                    magSLDs_p.append(i)
        ...
        # nuciSLDs
        nuciSLDs_p = []
        if self.nuciSLDs.any(): # check if nuciSLDs doesn't only contain zeros - if it does, we don't need parameters.
            for k, i in enumerate(self.nuciSLDs):
                if isinstance(i, Parameter):
                    nuciSLDs_p.append(i)
    ...
    def SLD_offset(self):
        ...
        if self.orientation in ('front'):
            ...
            sldprof_offset = -(zend_front - np.cumsum(self.thicks)[-1])
    ...
    def _gen_sld_profile(self, max_delta_z):
        ...
        # use a bit of refnx's slab code...
        # init a 2D array (Nlayers, 5)
        ...
        # use a bit of refnx's sld profile code
